                ...
                <table id="tableTrash" class="table table-striped table-row-bordered border rounded" style="width:100%">
                    <thead>
                        <tr class="fw-semibold fs-6 text-muted">
                            ...
                            <th class="border border-1 p-3">Username</th>
                            <th class="border border-1 p-3">Email</th>
                        ...
                        @foreach ($trashedData as $data)
                            <tr>
                                ...
                                <td class="border border-1 text-center">
                                    <a href="{{ route('user.restore', $data->id) }}" class="btn btn-icon btn-primary w-35px h-35px mb-sm-0 mb-3">
                                    ...
                                    <div class="modal fade text-start" tabindex="-1" id="confirmDelete{{ $data->id }}">
                                        <form action="{{ route('user.forceDelete', $data->id) }}">
                                            @csrf
                                            @method('delete')
                                ...
                                <td class="text-capitalize border border-1">{{ $data->email ?? '-' }}</td>
