...
@section('content')
    <div class="card mb-5 mb-xl-10">
        ...
        <div class="card-body">
            <a href="{{route('trans-date.create')}}">
                ...
                <table id="tableSKPD" class="table table-striped table-row-bordered border" style="width:100%">
                    <thead>
                        <tr class="fw-semibold fs-6 text-muted">
                            ...
                            <th class="border border-1">Tahun Periode</th>

                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($transdate as $data)
                            <tr>
                                <td class="border border-1 text-center">{{ $loop->iteration }}</td>
                                <td class="text-center border border-1">
                                    <a href="{{ route('trans-date.edit', $data->id) }}" class="btn btn-icon btn-primary w-35px h-35px mb-sm-0 mb-3">
                                    ...
                                    <div class="modal fade" tabindex="-1" id="confirmDelete{{ $data->id }}">
                                        <form action="{{ route('trans-date.destroy', $data->id) }}" method="POST">
                                            @csrf
                                            @method('delete')
                                ...
                                <td class="text-capitalize border border-1">{{ $data->trans_date}}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
