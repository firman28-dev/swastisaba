                ...
                <table id="tableSKPD" class="table table-striped table-row-bordered  border rounded" style="width:100%">
                    <thead>
                        <tr class="fw-semibold fs-6 text-muted">
                            ...
                            <th class="border border-1">Daftar Sesi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($categories as $item)
                            <tr>
                                ...
                                <td class="border border-1">
                                    <a href="{{ route('category-course.edit', $item->id) }}" class="btn btn-icon btn-primary w-35px h-35px mb-sm-0 mb-3">
                                    ...
                                    <div class="modal fade text-start" tabindex="-1" id="confirmDelete{{ $item->id }}">
                                        <form action="{{ route('category-course.destroy', $item->id) }}" method="POST">
                                            @csrf
                                            @method('delete')
                                ...
                                <td class="border border-1">
                                    <a href="{{ route('category-course.show', $item->id) }}" class="btn btn-icon btn-info w-35px h-35px mb-sm-0 mb-3">
                                        <div class="d-flex justify-content-center">
