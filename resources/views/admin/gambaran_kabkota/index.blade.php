    ...
    <div class="card mb-5 mb-xl-10">
        <div class="card-header justify-content-between">
            <div class="card-title">
                <h3>Gambaran Umum KabKota</h3>
                
            </div>
        </div>
        <div class="card-body">
            <a href="{{route('gambaran-kabkota.create')}}">
                ...
                <table id="tableSKPD" class="table table-striped table-row-bordered gy-5 gs-7 border rounded" style="width:100%">
                    <thead>
                        <tr class="fw-semibold fs-6 text-muted">
                            ...
                            <th class="w-300px border-1 border text-capitalize">Nama gambaran umum</th>
                            <th class="border-1 border">Keterangan</th>

                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($gambaran_kabkota as $data)
                            <tr>
                                <td class="text-center border-1 border">{{ $loop->iteration }}</td>
                                <td class="border-1 border">
                                    <a href="{{ route('gambaran-kabkota.edit', $data->id) }}" class="btn btn-icon btn-primary w-35px h-35px mb-sm-0 mb-3">
                                    ...
                                    <div class="modal fade" tabindex="-1" id="confirmDelete{{ $data->id }}">
                                        <form action="{{ route('gambaran-kabkota.destroy', $data->id) }}" method="POST">
                                            @csrf
                                            @method('delete')
