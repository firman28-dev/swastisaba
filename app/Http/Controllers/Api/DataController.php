...
class DataController extends Controller
{
    ...
    public function viewCapaiankelembagaanMarged(){
        $loginResponse = Http::asForm()->post('https://sipantas.kemkes.go.id/AP_Data/token', [
            'username' => 'kotapayakumbuh',
            'password' => 'Payakumbuh123!@#',
            'grant_type' => 'password',
    ...
    public function viewTatananPerkabkota2024(){
        $loginResponse = Http::asForm()->post('https://sipantas.kemkes.go.id/AP_Data/token', [
            'username' => 'kotapayakumbuh',
            'password' => 'Payakumbuh123!@#',
            'grant_type' => 'password',
    ...
    public function viewTatananAllKabkota(){
        $loginResponse = Http::asForm()->post('https://sipantas.kemkes.go.id/AP_Data/token', [
            'username' => 'kotapayakumbuh',
            'password' => 'Payakumbuh123!@#',
            'grant_type' => 'password',
        ...
        return response()->json([
            'status' => 'done',
            'token' => $token,
            'results' => $resultPerDistrict,
        ]);
    }

    //indikator tatanan versi clear
    public function viewTatananPerkabkotaMerged(){
        $loginResponse = Http::asForm()->post('https://sipantas.kemkes.go.id/AP_Data/token', [
            'username' => 'kotapayakumbuh',
            'password' => 'Payakumbuh123!@#',
            'grant_type' => 'password',
        ...
        foreach ($questions as $q) {
            ...
            //     'capaian_sebelumnya' => $answerPrev?->achievement ?? '-',
            //     'file_capaian' => $dokumen[1]['upload']['url'] ?? $dokumen[0]['upload']['url'] ?? '',
            //     'file_capaian_sebelumnya' => $dokumenPrev[1]['upload']['url'] ?? $dokumenPrev[0]['upload']['url'] ?? '',
            //     'penjelasan_kabupaten' => $answer?->comment ?? '-',
            //     'penjelasan_provinsi' => $answer?->comment_prov ?? '-',
            //     'nilai_mandiri' => $answer?->_q_option?->score ?? 0,
            //     'nilai_provinsi' => $answer?->_q_option_prov?->score ?? 0,
